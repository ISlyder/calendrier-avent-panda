export interface Calendar {
  day: number;
  text: string;
  img: string;
}

export const DATA: Calendar[] = [
  {
    day: 1,
    text: 'Coucou ma chérie ! Aujourd\'hui c\'est le premier jour de décembre et je t\'ai réservé une petite surprise ! Chaque jour, tu pourras ouvrir une case de ce calendrier de l\'avent pour découvrir un mot, une photo ou... LES DEUX. Aujourd\'hui j\'ai mis une de nos première photo tous les deux. Si tu te souviens c\'était à Essaouira, après mon arrivée à Marrakech. J\'espère que ça te plaira. Je t\'aime très fort !',
    img: 'assets/01.JPEG'
  },
  {
    day: 2,
    text: 'Jour 2 : Hier tu m\'as appris la meilleure nouvelle : tu as eu ton visa ! J\'ai tellement hâte de te retrouver ! J\'espère que tu vas aimer passer Noël avec moi et ma famile. Aujourd\'hui j\'ai décidé de mettre une autre photo à Essaouira pour rester dans la chronologie et aussi parce que je la trouve très belle. Je te fais pleins de BISOUS !',
    img: 'assets/02.JPG'
  },
  {
    day: 3,
    text: 'A partir d\'aujourd\'hui, j\'arrête la chronologie et je te remémore des moments que j\'ai aimé. En espérant que tu les as apprécié aussi 😘',
    img: 'assets/03.jpeg'
  },
  {
    day: 4,
    text: 'J\'espère que tu te souviens de toutes les photos que j\'ai pu mettre jusqu\'à là 😜Aujourd\'hui j\'ai simplement envi de te rappeler qu\'il ne reste que 19 jours avant que tu sois en France 🇫🇷🤞🏽J\'ai tellement hâte !!!!! Hâte de te retrouver, hâte de te prendre dans mes bras, hâte de t\'embrasser 😘',
    img: 'assets/04.jpeg'
  },
  {
    day: 5,
    text: 'Tu me manques ❤️❤️Passe une bonne journée chérie 😘',
    img: 'assets/05.JPEG'
  },
  {
    day: 6,
    text: 'Hbibaaaaaa 🥰❤️',
    img: 'assets/06.JPEG'
  },
  {
    day: 7,
    text: '',
    img: ''
  },
  {
    day: 8,
    text: '',
    img: ''
  },
  {
    day: 9,
    text: '',
    img: ''
  },
  {
    day: 10,
    text: '',
    img: ''
  },
  {
    day: 11,
    text: '',
    img: ''
  },
  {
    day: 12,
    text: '',
    img: ''
  },
  {
    day: 13,
    text: 'Today is thirteenth day of December !',
    img: ''
  },
  {
    day: 14,
    text: 'Today is fourteenth day of December !',
    img: ''
  },
  {
    day: 15,
    text: 'Today is fifteenth day of December !',
    img: ''
  },
  {
    day: 16,
    text: 'Today is sixteenth day of December !',
    img: ''
  },
  {
    day: 17,
    text: 'Today is seventeenth day of December !',
    img: ''
  },
  {
    day: 18,
    text: 'Today is eighteenth day of December !',
    img: ''
  },
  {
    day: 19,
    text: 'Today is nineteenth day of December !',
    img: ''
  },
  {
    day: 20,
    text: 'Today is twentieth day of December !',
    img: ''
  },
  {
    day: 21,
    text: 'Today is twenty-first day of December !',
    img: ''
  },
  {
    day: 22,
    text: 'Today is twenty-second day of December !',
    img: ''
  },
  {
    day: 23,
    text: 'Today is twenty-third day of December !',
    img: ''
  },
  {
    day: 24,
    text: 'Today is twenty-fourth day of December !',
    img: ''
  },
  {
    day: 25,
    text: 'Today is twenty-fifth day of December !',
    img: ''
  }
]
